...
;       charthick = array of char thickness numbers
...
;-
pro spu_legend,help=help,items,linestyle=linestyle,psym=psym,vectorfont=vectorfont $
  ...
  ,data=data,normal=normal,device=device,charthick=charthick $
   ,symsize=symsize,thick=thick,clear=clear,clrclear=clrclear
;
;	=====>> HELP
;
on_error,2
...
if n_elements(clrclear) eq 0 then cclr = -1 else cclr=clrclear(0)
if n_elements(charthick) eq 0 then chthk= 1 else chthk=charthick(0)
...
;    if (num eq 1) and vertical then xp = x + xt/2	; IF 1, CENTERED.
  ...
  xyouts,x,y,items[i],width=width,/norm,color=textcolors[i],size=charsize,align=xalign,charthick=chthk
  x = x + width*xsign
...
;	=====>> CLEAR AREA
	...
	corners = [x,y+ywidth,xend,y]
 	polyfill,[x,xend,xend,x,x],y + [0,0,ywidth,ywidth,0],/norm,color=cclr
