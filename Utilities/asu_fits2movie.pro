pro asu_fits2movie, fitsdir, windim = windim, visdir = visdir, fps = fps, img_name = img_name
...
foreach fits, files_in_all, i do begin
    ...
    outfile = visdir + path_sep() + img_name + string(i, FORMAT = '(I06)') + '.png'
    win.Save, outfile, width = windim[0], height = windim[1], bit_depth = 2
    win.Close
endforeach

to_filename = visdir + path_sep() + 'video.mp4'
hmi_mask = visdir + path_sep() + img_name + '%06d.png'
