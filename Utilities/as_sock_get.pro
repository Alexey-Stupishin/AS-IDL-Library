...
;-
;-----------------------------------------------------------------  
function as_sock_get_callback, status, progress, data  
...
pro as_sock_get_main,url,out_name,clobber=clobber,local_file=local_file,no_check=no_check,$
  ...
  no_dir_check=no_dir_check, no_rename = no_rename                
...
if is_number(progress) && ~is_pyidl() then begin
 if ~chunked && (bsize ne 0) && (progress gt 0.) then begin
  ...
  if allow_windows() && (bar lt 100.) then begin
   callback_function='as_sock_get_callback'
   init=bar
...
if ~keyword_set(no_rename) && is_string(disposition) then begin
...
pro as_sock_get,url,out_name,local_file=local_file,_ref_extra=extra,$
                     status=status,err=err,cancelled=cancelled,$
                     background=background, no_rename = no_rename 
...
for i=0,np-1 do begin
 as_sock_get_main,url[i],out_name[i],local_file=lfile,err=err,status=status,$
               cancelled=cancelled, no_rename = no_rename, _extra=extra
